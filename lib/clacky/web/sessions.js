  ...
  return {
    ...
    _getProgressState(id) {
      ...
      if (!Sessions._sessionProgress[id]) {
        Sessions._sessionProgress[id] = { el: null, interval: null, startTime: null, type: null, displayText: null, metadata: null };
      }
    ...
    // Returns null if there are no token numbers in metadata.
    _formatTokenSuffix(metadata) {
      ...
      const inStr  = (input == null || input <= 0) ? "—" : Sessions._compactTokenCount(input);
      const outStr = output == null ? "0" : Sessions._compactTokenCount(output);
      return `↑${inStr} ↓${outStr} tokens`;
    },

    // Compose the live progress line: "<text>… (Ns · ↑in ↓out tokens)"
    _composeProgressLine(displayText, startTime, metadata) {
    ...
    _attachProgressUI(id) {
      ...
      el.textContent = Sessions._composeProgressLine(state.displayText, state.startTime, state.metadata);
      messages.appendChild(el);
      state.el = el;
      _scrollToBottomIfNeeded(messages);
      ...
      state.interval = setInterval(() => {
        if (state.el) {
          state.el.textContent = Sessions._composeProgressLine(state.displayText, state.startTime, state.metadata);
        }
      }, 250);
    },
    ...
    showProgress(text, progress_type = "thinking", metadata = {}, startedAt = null) {
      ...
      if (existing && existing.el) {
        ...
        existing.metadata    = metadata || {};
        existing.el.textContent = Sessions._composeProgressLine(newDisplayText, newStartTime, metadata);
        if (existing.interval) clearInterval(existing.interval);
        existing.interval = setInterval(() => {
          if (existing.el) {
            existing.el.textContent = Sessions._composeProgressLine(existing.displayText, existing.startTime, existing.metadata);
          }
        }, 250);
        _scrollToBottomIfNeeded($("messages"));
        return;
